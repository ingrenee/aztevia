 ...
 ?> <!--  content receta home -->
	<article id="post-<?php the_ID(); ?>" <?php post_class(' color-3 '); ?>>
		...
		<?php else : ?>
			...
			<p><?PHP echo get_post_meta($post->ID,'wpcf-texto-portada-post',TRUE)?></p>
			
		<?php endif; // is_single() ?><a href="<?php the_permalink(); ?>" class="post-link color-2" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'wp-foundation' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"> <span class="corner"></span>Leer el post</a>
		</header><!-- .entry-header -->
